...
const buildLimitQueryParams = (offset, limit, fields) => {
  const advancedRequestOptions = {
    queryParams: ''
  };
  if (offset) {
    requestOpts.queryParams += 'offset=' + offset;
  }
  if (limit) {
    requestOpts.queryParams += 'limit=' + limit;
  }
  if (fields) {
    requestOpts.queryParams += 'fields=' + fields.join('&fields=');
  }
  return advancedRequestOptions;
}
...
module.exports = class RestTopic extends RestUtil {
  ...
  createReportInstance(objectId, body, offset, limit, fields) {
    ...
    const requestOpts = buildLimitQueryParams(offset, limit, fields);
