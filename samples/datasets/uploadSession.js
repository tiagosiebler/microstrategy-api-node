...
(async ()=> {
  ...
  try {
    ...
    const tableFormat = {
      ...
      updatePolicy: 'REPLACE',//[ ADD, UPDATE, UPSERT, REPLACE ]
      orientation: 'ROW',// or column
      columnHeaders: ['ID','SALES']
    };
    ...
    const uploadSessionId = await DatasetsAPI.createUploadSession(targetDatasetId, tablesFormatting);
    console.log('Have upload session with ID: ', uploadSessionId);

    console.log('publish: ', await DatasetsAPI.publishUploadSessionDataset(targetDatasetId, uploadSessionId));

    console.log('status: ', await DatasetsAPI.getUploadSessionStatus(targetDatasetId, uploadSessionId));
